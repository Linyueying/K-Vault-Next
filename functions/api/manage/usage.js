 ...
 *   - CF_API_TOKEN     (必填) API Token，需要 "账户分析:读取" 和 "Workers R2 存储:读取" 权限
...
export async function onRequestGet(context) {
  ...
  if (!kvNamespaces.length && !r2Buckets.length) {
    return Response.json({ success: false, configured: false, error: '未检测到 KV 或 R2 绑定，请检查 Pages 项目的绑定配置', plan: null, kv: null, r2: [], r2Available: false, date: '', resetAt: '' }, { status: 503 });
  }

  if (!accountId || !apiToken) {
    return Response.json({ success: false, configured: true, error: '缺少 CF_ACCOUNT_ID 或 CF_API_TOKEN，无法查询用量数据', plan: null, kv: null, r2: [], r2Available: r2Buckets.length > 0, date: '', resetAt: '' }, { status: 503 });
  }
  ...
  try {
    ...
    const resetLabel = plan.type === 'free' 
      ? 'KV 每日 00:00 UTC 重置 · R2 按月重置' 
      : (billingPeriod ? `KV 计费周期 ${billingPeriod.start} 至 ${billingPeriod.end}，超出按量付费 · R2 按月计费` : 'KV 按月计费，超出按量付费 · R2 按月计费');
