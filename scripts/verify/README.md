...
### `folder-delete.mjs` —— admin 目录删除：乐观更新 + 秒级响应回归

```bash
# 前置：wrangler pages dev 跑在 8099，并先用 _seed-kv.mjs 造两个目录
#   verify-deltest      （成功路径用例）
#   verify-deltest-rb   （失败回滚用例，需独立数据：成功用例会把目录删掉）
BASE=http://localhost:8099 node scripts/verify/folder-delete.mjs
```

断言 6 项：**A** 点确认后目录项从目录树消失的耗时（乐观更新，阈值 300ms）、
**B** 删除后不再发出 `list?limit=1000` 全量请求、**C** DELETE 请求带 `recursive=1`、
**D** 后端目录列表已移除该项、**E** 无 JS 报错、**F** 后端失败时目录项能回滚到界面。

> **这个脚本的由来**：删除目录的语义是「把目录内文件移回根目录」（文件不删、
> 直链不失效），后端要逐个改写目录内所有文件的 metadata。早期实现有两处问题：
>
> 1. **前端零乐观更新** —— 目录项要等「后端删完 + 两次全量刷新」全部完成才消失，
>    用户感受是「点了删除，目录一直挂在界面上不动」。
> 2. **后端串行 `for...await`** —— 每个文件一次完整网络往返，耗时随文件数线性增长。
>    线上 Cloudflare KV 单次写是跨网络往返，300 个文件约 **9 秒**（本地 SQLite
>    同步写只有 0.7 秒，**所以本地数字掩盖了这个 bug，必须看并发微基准或线上**）。
>
> 修复后：前端确认即消失（实测 2ms）；后端改 `Promise.allSettled` 分批并发
> （16/批），同样 300 文件在 30ms/次往返的模型下从 9042ms 降到 573ms（**15.8×**）。
>
> ⚠️ 本地验证的局限：本地 KV 是 SQLite 同步写，"并发 vs 串行"几乎没有差异。
> **不要用本地 DELETE 耗时去证明并发优化有效**，要用模拟网络延迟的微基准，
> 或直接看线上表现。本脚本负责的是「前端乐观更新 + 无冗余刷新 + 回滚」这三点。
