...
| `BASIC_USER` | 管理后台用户名（未配置时管理接口 fail-closed 返回 503） | 可选 |
| `BASIC_PASS` | 管理后台密码（同上） | 可选 |
...
- `GET /api/manage/login` 是 `/api/manage/**` 下唯一豁免的路由：未登录时 `302` 跳 `/login.html?redirect=%2Fadmin.html`，已登录时跳 `/admin.html`，否则无法登录。
- `POST /api/upload-from-url` 与 `POST /upload` 采用同一套管理员/访客门槛：管理员放行；访客需 `GUEST_UPLOAD=true` 且满足单文件与每日次数限制；否则拒绝（此前该接口在 Cloudflare Pages 上**完全无鉴权**）。

另外 `GET /api/status` 按登录状态分级返回：未登录访客只拿到「各后端是否可用（`enabled`/`configured`）+ 访客上传配置 + 各后端上传上限 + 能力清单」；连通性探测结果、后端错误详情、机器人身份等诊断信息仅管理员可见，且未登录请求不会触发任何后端连通性探测。
