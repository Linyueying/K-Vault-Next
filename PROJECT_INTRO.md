...
> `server/` 目录作为上游 Node.js/Hono 运行时源码保留（供参考与二次开发），**不再是受支持的部署目标**。
...
- 保留的 Node.js/Hono 运行时（`server/`，非部署目标）：提供与 Pages 对齐的后端接口；其 Nginx 容器编排与代理配置已随自托管部署路径一并移除

### 3. 数据层

- Cloudflare Pages：KV/R2 等绑定承载元数据与对象数据
- 保留的 Node 运行时（`server/`，非部署目标）：SQLite 存储文件元数据、存储配置、会话、访客计数、分片任务、API Token、Paste 等数据
- 可选 Redis：用于应用设置存储
...
- 分享入口：`/s/:slug` 短分享链；签名分享链接（`POST /api/share/sign` + `/share/:id`）只存在于保留的 Node 运行时，Pages 部署下不可用
...
- 本仓库不提供 Docker / Nginx 自托管部署路径；`server/` 仅作为上游 Node 运行时源码保留，不再是受支持的部署目标
