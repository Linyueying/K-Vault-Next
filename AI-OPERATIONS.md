...
26. **跟手拖拽要关掉 transition** —— 只做「逐格吸附」是跳跃、不是跟手。
    正确做法：锁定横向后给拖拽态 `transition: none`，每帧把连续位置**直接写 DOM**
    （不走响应式，否则每帧 setState 掉帧）；松手先撤掉内联值、恢复 transition，
    让 CSS 弹簧完成吸附回弹。（⚠️ 本条早期版本还建议「叠加像素偏移」，那是错的，
    见铁律 27 —— 已更正。）
27. **位移必须单变量驱动，禁止「整数索引 + 像素偏移」双表达叠加** ——
    这是「滑着滑着开始瞬移」的根因。当时位移写成
    `translateX = i * 步长 + dragX`：换格时 `i` 跳一整格，而 `dragX` 已累计了
    这段位移 → **重复相加**，指示器瞬移一整个槽位（实测 +102~125px）。
    正解汲取自 `Kyant0/AndroidLiquidGlass` 的 `DampedDragAnimation.kt` /
    `LiquidBottomTabs.kt`：**只保留一个浮点位置真源**（单位=「格」，可为小数），
    `位置 = pos × 步长`，拖拽即 `pos = pos0 + 位移/步长`，松手弹簧吸附到整数格；
    速度只作派生量驱动形变、不参与定位。落地为 `--dock-pos`（替代 `--dock-i`
    + `--dock-drag-x`），且常态与拖拽态**共用同一条 transform 公式**。
    验证要「细粒度连续采样 + 相邻位移上限」（`dock-indicator.mjs` 以 2px 步长
    采 188 点，断言相邻位移 ≤ 8px；旧 bug 实测 125.5px，必被抓住）。
    上限**不要**用「槽宽 × 系数」——槽宽约 102px，取 2.5×=309px 会放过 125px
    的瞬移（本次已踩坑）。要按**手指步长**定阈值。
