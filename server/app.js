...
function createApp() {
  ...
  // Management surface fails closed: with no BASIC_USER/BASIC_PASS configured
  // nobody can ever authenticate, so previously every protected route (manage
  // delete/rename/move, storage config CRUD, settings, share signing) was open
  // to the public internet. This matches requireAdminAuth() below and the
  // documented policy for the admin API.
  function requireAuth(c) {
    ...
    if (!authService.isAuthRequired()) {
      return jsonError(
        ...
        'Set BASIC_USER and BASIC_PASS to enable the management API. It fails closed.'
      );
    }
